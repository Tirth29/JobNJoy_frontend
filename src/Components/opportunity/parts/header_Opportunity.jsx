...
const Header = () => {
  ...
  const {loading, company } = useSelector((state) => state.user);
  console.log(company);
  ...
  const [ComapnyInfo, SetCompanyInfo] = useState({
    ...
    MoreInfo:
      "this comapany want Web designer and app Developers who have experience in react and node js",
  });
  ...
  return (
    <>
      <div className=" ">
        <div className=" text-2xl m-1 text-center text-white underline bg-blue-400 rounded-md p-2">
          Opportunities:
        </div>
        <DialogBox
        ...
        <div className=" my-10 rounded flex justify-around sm:mx-20">
          { render ? ( company?.map((company) => (
            <div
              className="rounded flex w-1/2 justify-center "
              onClick={() =>
                handleClik(
                  company.name,
                  company.hiring_domain,
                  company.description,
                  company.domain,
                  company.salary
                )
